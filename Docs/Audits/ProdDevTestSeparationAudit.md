...
Date: 2026-06-30
Scope: prod/dev/test separation audit before final UI work. Pass 1 implementation now gates debug/sandbox ownership and controller exec commands for non-shipping builds only. No gameplay, UI-final, map, Blueprint, DataAsset, or validation script changes were made.

## 1. Statut global

### Verdict court

Risque actuel apres Pass 1: **moyen avant UI finale**.
...
Remaining risk before final UI comes from later passes: `FGambitDebug...` runtime feedback naming, the default `GameInstanceClass` path under `Blueprints/Dev`, stale legacy/debug content references, and the temporary PC shell API cleanup.
...
| Production -> Debug-named feedback types | **Yes, by naming/API** | Runtime score, gold, shop, effect, dice snapshots use `FGambitDebug...` types. Behavior is runtime feedback, but naming and API are debug-polluted. |
...
It can stay in runtime for V0.1 because the current target is PC keyboard/mouse and docs already define it as a minimal playable shell. It should not become the final UI foundation without an API cleanup because it currently consumes `FGambitDebug...` snapshots and `DebugText` fields that should become stable production presentation/feedback APIs.
...
| `Source/GrandpaGambit/Public/Game/Flow/GambitRoundFlowComponent.h` and `.cpp` | Production Runtime with debug-named feedback | Production Runtime | Rename feedback API later | Medium | Generates score/gold/shop lines with `FGambitDebug...`; behavior is production feedback. |
| `Source/GrandpaGambit/Public/Game/Flow/GambitRoundEffectPipeline.h` and `.cpp` | Production Runtime with debug-named feedback | Production Runtime | Rename feedback API later | Medium | Appends effect/gold/shop feedback using debug-named fields. |
| `Source/GrandpaGambit/Public/Game/Flow/GambitTargetSelectionService.h` and `.cpp` | Production Runtime with debug-named UI text | Production Runtime | Rename `DebugText` later | Medium | `DebugText` is actually player-facing summary/invalid reason preview text. |
| `Source/GrandpaGambit/Public/Core/Types/GambitDebugTypes.h` | Runtime feedback and snapshots with debug naming | Production Runtime feedback/presentation types | Rename or alias to stable names | Medium | Used broadly by runtime and shell. Not inherently test-only, but naming is misleading. |
| `Source/GrandpaGambit/Public/Core/Types/GambitTargetSelectionTypes.h` | Production Runtime with debug-named text fields | Production Runtime | Rename `DebugText` later | Medium | Field is used by PC shell and target service as presentation text. |
| `Source/GrandpaGambit/Public/Items/Effects/GambitEffectExecutionTypes.h` | Production Runtime with debug-named feedback arrays | Production Runtime | Rename feedback arrays later | Medium | Effect execution context stores score/gold/shop/effect feedback. |
| `Source/GrandpaGambit/Public/Players/Components/GambitPlayerRoundStateComponent.h` and `.cpp` | Production Runtime with debug-named feedback storage | Production Runtime | Rename feedback storage later | Medium | Stores round breakdowns for runtime display and audit. |
| `Source/GrandpaGambit/Public/Players/States/GambitPlayerState.h` and `.cpp` | Production Runtime with debug-named snapshot API | Production Runtime | Rename/replace snapshot API before final UI | Medium | `BuildDebug...` and `GetDebug...` are used by shell/viewmodel. |
| `Source/GrandpaGambit/Public/Players/Components/GambitEconomyComponent.h` and `.cpp` | Production Runtime with debug-named gold lines | Production Runtime | Rename feedback output later | Medium | Gold breakdown lines are useful production feedback. |
| `Source/GrandpaGambit/Public/UI/Widgets/GambitPCShellWidget.h` and `.cpp` | Temporary PC Shell / Production UI V0.1 | Temporary PC Shell until final UI replaces it | Keep for V0.1, do not use as final UI base without API cleanup | Medium | Enables current playable loop; consumes debug-named snapshots. |
| `Source/GrandpaGambit/Public/UI/ViewModels/GambitMatchViewModel.h` and `.cpp` | UI API with debug snapshot naming | Production UI API | Replace with stable UI snapshots | Medium | Future UI should not consume `FGambitDebugPlayerSnapshot` as its main contract. |
...
| `Source/GrandpaGambit/Public/Debug/GambitDevMatchSandboxTypes.h` | Sandbox data types | Sandbox / Dev Tools | Keep with sandbox | Medium | Depends on debug snapshot types for inspector. |
...
| Runtime feedback systems | `Core/Types/GambitDebugTypes.h` | Runtime state and execution contexts use debug-named types | Not test-only behavior, but final UI API is semantically polluted. | Rename/migrate to `RoundFeedback`, `ScoreBreakdown`, `PlayerSnapshot`, or similar stable runtime naming. | P1 |
| `UGambitMatchViewModel` | `FGambitDebugPlayerSnapshot` | UI-facing API returns debug snapshot types | Future final UI would depend on debug semantics. | Replace with stable production UI snapshot structs and `Request...` command APIs. | P1 |
| `FGambitTargetSelectionRequest/Option` | `DebugText` | Runtime target selection API exposes debug-named presentation text | Current text is useful UI copy, not debug-only data. | Rename to `SummaryText`, `ReasonText`, or `PreviewText` during feedback API cleanup. | P2 |
...
- Replace debug-named runtime feedback types with stable production feedback/presentation names.
- Replace `UGambitMatchViewModel::BuildDebugPlayerSnapshots()` with stable UI snapshots.
- Rename target selection `DebugText` fields to production presentation names.
...
- Old aliases for `FGambitDebug...` types, if a compatibility migration is used.
...
- `FGambitDebug...` feedback structs: keep behavior, rename semantics later.
...
Use this prompt only if the validation gate for this audit passes. If the gate fails, run a minimal correction pass for the validation failure first.

```text
Tu travailles sur Grandpa Gambit, projet Unreal Engine 5 C++.

Objectif de cette passe:
Executer uniquement la Pass 1 de `Docs/Audits/ProdDevTestSeparationAudit.md`: isoler les outils Debug/Sandbox du runtime production.

Contraintes:
- Ne pas faire la Pass 2+.
- Ne pas renommer les `FGambitDebug...` feedback types dans cette passe.
- Ne pas faire UI finale.
...
2. Auditer les references exactes de:
   - `UGambitMatchDebugComponent`
   - `UGambitDevMatchSandboxComponent`
   - `UGambitDebugAutoPlayer`
   - `UFUNCTION(Exec)` dans `AGambitPlayerController`
3. Modifier uniquement ce qui est necessaire pour que le runtime production n'instancie pas et n'expose pas les outils debug/sandbox de facon inconditionnelle.
4. Les commandes debug/dev peuvent rester disponibles en editor/dev/non-shipping, mais doivent etre clairement gatees ou deplacees.
5. Ne pas modifier les regles de gameplay, scoring, economie, shop, dice, target selection.
6. Mettre a jour `Docs/Audits/ProdDevTestSeparationAudit.md` avec le resultat exact de la Pass 1.
7. Lancer:
...
- fichiers modifies;
- resume technique;
- commandes debug/dev encore disponibles et sous quelles conditions;
- confirmation que prod ne depend plus directement des composants debug/sandbox;
- resultat complet du gate.
```

## 10. Taches restantes apres la premiere passe
